...
class HtmlParser:
    ...
    def select_attr(self, css_selector: str, attr: str, **kwargs: Any) -> list[str]:
        ...
        return [
            element.get(attr) for element in elements if element.get(attr) is not None
        ]
    ...
    def find_elements(
        ...
        results = []
        for tag in elements:
            find_function = tag.find if single else tag.find_all
            matches = (
                [find_function(name=element_name)]
                if single
                else find_function(name=element_name)
            )
            for match in matches:
                results.append(match if not attribute else match.get(attribute))
        return results
